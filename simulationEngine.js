...
async function triggerInactiveAiAction(charId) {
    ...
    const systemPrompt = `
        ...
        # 核心规则
        1.  你的行动【必须】严格符合当前的真实时间。
        2.  你的回复【必须】是一个JSON数组，每个对象代表一个动作。你通常应该只执行【一项】最符合你当前状态的行动。
        ...
        -   发消息: \`[{"type": "text", "name": "${chat.name}", "content": "你想对用户说的话..."}]\`
        -   发文字动态: \`[{"type": "create_post", "name": "${chat.name}", "postType": "text", "content": "动态的文字内容..."}]\`
        -   发图片动态: \`[{"type": "create_post", "name": "${chat.name}", "postType": "image", "publicText": "(可选)配图文字", "imageDescription": "对图片的详细描述"}]\`
